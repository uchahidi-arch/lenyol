...
/** Convertit le texte brut (markdown simplifié) en HTML pour l'affichage */
function formatContenu(raw: string): string {
  return raw
    ...
    // Italique
    .replace(/\*(.+?)\*/g, '<em>$1</em>')
    // Blocs vides entre balises de titre → ne pas envelopper
    .split(/\n{2,}/)
    .map(block => {
      const trimmed = block.trim();
      if (!trimmed) return '';
      // Ne pas envelopper si c'est déjà un bloc HTML
      if (/^<(h[1-6]|p|ul|ol|li|blockquote|div)/.test(trimmed)) return trimmed;
      // Sauts de ligne simples dans un paragraphe → <br>
      return `<p>${trimmed.replace(/\n/g, '<br>')}</p>`;
    })
    .join('\n');
}

export default function ChroniqueePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  useEffect(() => {
    import('@/lib/supabase').then(async ({ supabase }) => {
      ...
      // 2. Charger les personnes liées (table article_persons optionnelle)
      try {
        ...
        // table article_persons pas encore créée — on ignore
      }
