...
const CATEGORIES = ['royaumes', 'lignées', 'ethnies', 'noms de familles'];
...
export default function ModifierRacinePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const [categorie, setCategorie]   = useState('royaumes');
  ...
  useEffect(() => {
    if (!authLoading && !canWrite) router.replace('/racines');
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .single()
      .then(({ data }) => {
        if (!data) { router.replace('/racines'); return; }
        setArticleId(data.id);
        setTitre(data.titre ?? '');
        setNewSlug(data.slug ?? '');
        setCategorie(data.categorie ?? 'royaumes');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push(`/racines/${newSlug.trim()}`);
  };
